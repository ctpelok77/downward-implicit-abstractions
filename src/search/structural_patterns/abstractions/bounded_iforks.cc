...
BoundedIforksAbstraction::BoundedIforksAbstraction() : IforksAbstraction() {

}

BoundedIforksAbstraction::BoundedIforksAbstraction(Domain* new_dom) : IforksAbstraction(new_dom->get_var()) {
	set_root_var_and_domain(new_dom);
}

BoundedIforksAbstraction::BoundedIforksAbstraction(IforksAbstraction* ifork, Domain* new_dom) : IforksAbstraction(new_dom->get_var()) {
	set_root_var_and_domain(new_dom);
	if (ifork->get_abstraction_creation_status() != DONE)
		return;
