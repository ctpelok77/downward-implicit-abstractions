...
VariablesProjection::VariablesProjection() : GeneralAbstraction() {

}

VariablesProjection::VariablesProjection(vector<int>& pattern) : GeneralAbstraction() {
	set_pattern(pattern);
}
...
void VariablesProjection::abstract_prevail(vector<Prevail> prevail, vector<Prevail>& prv, const vector<int>& abs_vars) {
	for (size_t it=0; it < prevail.size(); ++it) {
...
//	orig_vars.resize(var_count);
    ...
    for(int i = 0; i < var_count; i++)
          init_state_buffer[i] = p->get_initial_state()[ptrn[i]];
	...
	Problem* abs_prob = new Problem(var_name, var_domain, init_state_buffer, g, ops, no_axioms);
...
void VariablesProjection::abstract_action(const vector <int>& abs_vars, DOperator* op,	vector<DOperator*>& abs_op){
