...
double BinaryForks_ON::get_solution_value(const SPState &state) {
...
double BinaryForks_ON::get_h_val(int sigma, const SPState &eval_state) const {
	...
	if (sigma > 1) {
		...
		if (s  == infinity)
			return s;
		int w_coeff0 = static_cast<int>(ceil(sig));
		sol += (w_coeff0 * s);
	}
	if (sigma > 2) {
		...
		if (s  == infinity)
			return s;
		int w_coeff1 = static_cast<int>(floor(sig));
		sol += (w_coeff1 * s);
	}
