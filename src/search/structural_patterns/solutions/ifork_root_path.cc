...
//	cp->get_first_needed_pairs(first_needed_pairs);
	...
	for (size_t i=0; i < first_needed_pairs.size(); ++i) {
...
void IforkRootPath::set_path(vector<DOperator*>& p) {
	path = p;
}
...
void IforkRootPath::get_applicable_vals(int root_bound, vector<int>& vals) const {
	...
	vector<PrePost> pp;
	path[0]->get_var_pre_post(0,pp);
	assert(pp.size() == 1);
	val = get_value_for_var(0, pp[0].cond);
	if (-1 != val) {
		vals.push_back(val);
		return;
	}
	...
	for (size_t i=0; i < path.size(); ++i) {
		vector<PrePost> eff;
		path[i]->get_var_pre_post(0,eff);
		assert(eff.size() == 1);

		inapplicable[eff[0].post] = 0;
	}
...
SPLPConstraint* IforkRootPath::get_LP_constraint() {
	return lpc;
}

void IforkRootPath::set_LP_constraint(SPLPConstraint* c) {
	lpc = c;
}
