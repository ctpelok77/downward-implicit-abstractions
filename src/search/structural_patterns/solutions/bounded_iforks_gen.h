#ifndef STRUCTURAL_PATTERNS_SOLUTIONS_BOUNDED_IFORK_GEN_H
#define STRUCTURAL_PATTERNS_SOLUTIONS_BOUNDED_IFORK_GEN_H
...
class BoundedIfork: public SolutionMethod {
	...
	void create_non_dominating_paths(double* sol, std::vector<std::vector<int> >& to_keep, bool remove_dominating);
	double calculate_supp_cost(double* sol, int v, std::vector<int>& supp);
	void precalculate_path(double* sol, IforkRootPath* path);
	void sort_and_remove_dominating_paths(int root_val, std::list<IforkRootPath*>& paths, bool remove_dominating);

protected:
	std::vector<IforkRootPath*>* root_paths_values;
