...
BoundedIfork::BoundedIfork() {
	set_abstraction(new BoundedIforksAbstraction());
}
...
BoundedIfork::BoundedIfork(IforksAbstraction* ifork, Domain* abs_domain){
	set_abstraction(new BoundedIforksAbstraction(ifork, abs_domain));
}
...
BoundedIfork::~BoundedIfork() {
	...
	for (int root_val = 0; root_val < root_dom; root_val++) {
		for (size_t i=0; i < root_paths_values[root_val].size(); ++i) {
...
//	cout << "Solving the inverted fork" << endl;
	...
	for (int v = 1; v < var_num; v++) {
		...
		for (int a = 0; a < A_v_size; a++) {
			...
			A_v[a]->get_var_pre_post(v,pre_v);
			assert(pre_v.size() > 0);
			for (size_t p=0; p < pre_v.size(); ++p) {
				int pre = pre_v[p].pre;
				assert(pre==prv_v || pre == -1 || prv_v == -1);
				if (-1 == pre)
					pre = prv_v;
				int post = pre_v[p].post;
				assert(post != -1);

				const vector<Prevail> &cond = pre_v[p].cond;
				for (size_t co=0; co < cond.size(); ++co) {
					if (cond[co].var == v) {
						assert(pre == -1);
						pre = cond[co].prev;
					}
				}
				// Now all conditions are set - setting the respective values
		...
		for (int k=0;k<dom_size;k++) {
			for (int i=0;i<dom_size;i++) {
				for (int j=0;j<dom_size;j++) {
					...
					sol[d_indij] = min(sol[d_indij], sum_infinity(sol[d_indik],sol[d_indkj]));
				}
			}
		}
	}
...
//		cout << "For root value " << i << " from " << root_paths_vals[i].size() << " only left" << endl;
		...
		const vector<IforkRootPath*>& paths = root_paths_values[i];
		num_left_paths += paths.size();
		for (size_t j=0; j < paths.size(); ++j) {

			const vector<pair<int,int> > &first_needed =
				paths[j]->get_first_needed_pairs();

			for (size_t k=0; k < first_needed.size(); ++k) {
...
//	cout << "Sorting and removing dominating paths for root value " << root_val << endl;
	...
	vector<IforkRootPath*>& root_paths = root_paths_values[root_val];
	for (size_t i=0; i < root_paths.size(); ++i) {
		if (root_paths[i] != nullptr) {
			delete root_paths[i];
		}
	}
...
//					paths.erase(it);
	...
	for (list<IforkRootPath*>::iterator it=paths.begin(); it!=paths.end(); ++it) {
//		cout << "Keeping the path " << *it << endl;
		root_paths.push_back(*it);
	}
//	if (STATISTICS >= 2) {
//		cout << "Sorted vector of paths for root value "<< root_val << " of size " << root_paths.size()  << endl;
//	}
//	if (STATISTICS >= 3) {
//		for (vector<IforkRootPath*>::iterator it=root_paths.begin();
//											it!=root_paths.end(); ++it)
//			(*it)->dump();
//		cout << "#######################################################################"<<endl;
//	}
}
...
double BoundedIfork::get_solution_value(const SPState &state) {

//	cout << "Original state" << endl;
//	state.dump_fdr();
//	cout << "Getting abstract state" << endl;
	const SPState &abs_state = get_abstract_state(state);
//    for (size_t i = 0; i <= num_parents; ++i) {
//    	int orig_var = get_mapping()->get_original_var(i);
//        cout << "  #" << i << " [" << g_variable_name[orig_var] << "] -> "
//             << abs_state[i] << endl;
//    }
//	cout << "Done" << endl;
...
//	cout << "Getting the number of paths for initial root value " << root_zero << endl;
	const vector<IforkRootPath*>& root_paths = root_paths_values[root_zero];

	int num_paths = root_paths.size();

//	cout << "Got " << num_paths << " paths" << endl;
	double tmp_sol;
	// For each cycle free path:
	for (int p = 0; p < num_paths; p++) {
//		cout << "1" << endl;
		IforkRootPath* path = root_paths[p];
//		cout << "Path:" << path << endl;
//		path->dump();
		double sol = path->get_needed_cost();
//		cout << "Got path of cost " << sol << " when current minimum is " << min_sol << endl;
...
//		cout << "Going over all *supporting* parents" << endl;
		int first_needed_size = first_needed.size();
		for (int i = 0; i < first_needed_size; i++) {
			int from = abs_state[first_needed[i].first];
			int to = first_needed[i].second;
			if (from != to) {

				tmp_sol = solution->get_value(d_var(first_needed[i].first,from,to));
//				cout << "Cost of supporting parent change from " << from << " to " << to << " for parent " << first_needed[i].first << " is " << tmp_sol << endl;
				sol += tmp_sol;
...
//	cout << "Done going over the paths" << endl;
	return min_sol;
}
